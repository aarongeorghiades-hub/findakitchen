...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Root layout applies the "%s | FindAKitchen" template — do NOT hard-code the
  // brand suffix here.
  return {
    title: `${provider.name} — Temporary Kitchen Hire`,
    description: `${provider.name} offers temporary kitchen hire covering ${provider.coverage ?? regionLabel(provider.region_base)}. ${provider.notable_differentiators?.[0] ?? ""}`,
    alternates: {
  ...
  return (
    <>
      ...
      <div className="px-6 lg:px-12 py-12 space-y-12 max-w-5xl">
        ...
        {/* SOCIAL PROOF — rating numbers only; no outbound Trustpilot link */}
