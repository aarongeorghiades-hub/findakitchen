...
export default async function ComparePage({ params }: Props) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Compare", item: "https://findakitchen.co.uk/compare" },
      { "@type": "ListItem", position: 3, name: page.title, item: `https://findakitchen.co.uk/compare/${params.slug}` },
    ],
  };
      ...
      <section className="bg-[var(--charcoal)] pt-12 pb-16 px-6 lg:px-12 relative overflow-hidden">
        ...
        <div className="max-w-3xl relative z-10">
          <nav className="text-xs text-white/40 mb-6 flex items-center gap-1.5">
            ...
            <span>/</span>
            <a href="/compare" className="hover:text-white/60 transition-colors">Compare</a>
            <span>/</span>
          ...
          <p className="text-xs uppercase tracking-widest text-[var(--clay-light)] mb-4">
            Side-by-side comparison &middot; {Math.ceil((page.content?.split(" ").length || 800) / 200)} min read
          </p>
